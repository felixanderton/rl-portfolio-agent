...
def _momentum_return(prices: pd.Series, window: int) -> pd.Series:
    """
    Cumulative log return over `window` trading days:
        mom_t = log(P_t / P_{t-window})

    Equivalent to the sum of daily log returns over the lookback window.
    Captures medium- to long-term cross-sectional momentum signals.
    """
...
def _build_feature_matrix(prices_df: pd.DataFrame, window: int) -> pd.DataFrame:
    ...
    Column order: [asset0_logret, asset0_vol, asset0_meanrev, asset0_mom63, asset0_mom252,
                   asset1_logret, asset1_vol, asset1_meanrev, asset1_mom63, asset1_mom252, ...]

    The first 252 rows will contain NaNs from the 252-day momentum calculation;
    ...
    for ticker in prices_df.columns:
        ...
        mom63 = _momentum_return(p, 63)
        mom252 = _momentum_return(p, 252)
...
def load_data() -> dict:
    ...
    assert rows_dropped <= 252 + 2, (
        f"dropna() dropped {rows_dropped} rows from train — expected at most "
        f"254 (252-day momentum burn-in + 2). Possible mid-series NaNs in the downloaded data."
    )
